...
@SpringBootTest
@Import(KnockoutIntegrationTest.MockConfig.class)
@Sql(executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD, scripts = {
        "/seed/clean.sql",
        "/seed/cards-seed-data.sql"
})
class KnockoutIntegrationTest {
    ...
    void setUp() {
        ...
        deck2Id = createTestDeck(player2Id, "P2 Squirtle Deck",
                List.of(
    ...
    void shouldProcessKnockout() {
        ...
        assertFalse(setupP1.getHand().isEmpty());
        assertFalse(setupP2.getHand().isEmpty());
        ...
        UUID p2BenchBasicId = findSecondBasicInstance(setupP2, p2BasicId);
        if (p2BenchBasicId != null) {
            resp = executeAction(matchId, "SETUP_PLACE_BENCH", player2Id,
                    Map.of("cardInstanceId", p2BenchBasicId.toString()));
            assertTrue(resp.success(), "P2 place bench: " + errorMsg(resp));
        }
        ...
        // Play turn by turn, always attacking with P1
        ...
        if (state.isPendingKOReplacement()) {
            assertTrue(state.getStatus() == MatchStatus.ACTIVE, "Match should be active");
            assertEquals(player2Id, state.getKnockedOutPlayerId(), "P2 should have a KOd Pokemon");

            PlayerState p2AfterKO = findPlayer(state, player2Id);
            assertNull(p2AfterKO.getActivePokemon(), "P2 active should be null after KO");
            assertFalse(p2AfterKO.getBench().isEmpty(), "P2 should have bench Pokemon");

            UUID benchPkmId = p2AfterKO.getBench().get(0).getInstanceId();
            resp = executeAction(matchId, "CHOOSE_KO_REPLACEMENT", player2Id,
                    Map.of("benchPokemonInstanceId", benchPkmId.toString()));
            assertTrue(resp.success(), "KO replacement: " + errorMsg(resp));

            state = gameEngine.loadState(matchId);
            assertFalse(state.isPendingKOReplacement(), "KO replacement cleared");
            assertNotNull(findPlayer(state, player2Id).getActivePokemon(), "P2 should have a new active");
            assertTrue(state.getStatus() == MatchStatus.ACTIVE, "Match should continue after replacement");
        } else {
    ...
    private UUID findSecondBasicInstance(PlayerState player, UUID excludeInstanceId) {
        var basicCardIds = List.of("seed-pikachu", "seed-pikachu-ex", "seed-charmander", "seed-squirtle");
        return player.getHand().stream()
                .filter(c -> basicCardIds.contains(c.getCardDefinitionId()))
                .filter(c -> !c.getInstanceId().equals(excludeInstanceId))
                .findFirst()
                .map(c -> c.getInstanceId())
                .orElse(null);
    }
