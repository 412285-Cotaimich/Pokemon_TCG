...
class ReviveToDeckResolverTest {
    ...
    private ReviveToDeckResolver resolver;
    ...
    @BeforeEach
    void setUp() {
        resolver = new ReviveToDeckResolver();
    ...
    @Test
    void resolve_shouldRevivePokemonFromDiscardToBench() {
        ...
        assertEquals(5, bench.get(0).getEnteredTurnNumber());
    ...
    void resolve_shouldPublishEvent() {
        ...
        assertEquals(GameEventType.POKEMON_REVIVED.name(), event.getType());
        assertEquals("pkm-1", event.getPayload().get("cardDefinitionId"));
        assertNotNull(event.getPayload().get("pokemonInstanceId"));
    }

    @Test
    void getType_shouldReturnREVIVE_TO_DECK() {
        assertEquals(EffectType.REVIVE_TO_DECK, resolver.getType());
    }
